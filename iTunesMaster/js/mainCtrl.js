...
$scope.getSongData = function () {
  return itunesService.getData($scope.artist)
    .then(function (response) {
      var songs = [];
      var results = response.data.results;

      results.forEach(function (songele) {
        songs.push({
          AlbumArt: songele.artworkUrl30,
          Artist: songele.artistName,
          Collection: songele.collectionName,
          CollectionPrice: songele.collectionPrice,
          Play: songele.trackViewUrl,
          Type: songele.kind
        });
      });
